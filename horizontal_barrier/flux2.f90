!     =====================================================
    ...
    amdq,apdq,cqxx,bmasdq,bpasdq,rpn2,rpt2,use_fwave,vert,horz,&
    bar_index_i,bar_ht,bar_loc,alpha)
!     =====================================================
...
!f2py intent(callback) rpn2,rpt2
    ...
    logical :: L2R,R2L
    logical :: limit, use_fwave, vert,horz
    dimension wave_wall(3,3),s_wall(3),amdq_wall(3),apdq_wall(3)
    ! dimension q_hboxes(3,4)
    integer :: bar_index,bar_index_i
    real(8) :: bar_height,bar_ht,bar_l
    real(8) :: dtdx,lamb, qh_L(3),auxh_L,qh_R(3),auxh_R
...
!     # Set qadd for the donor-cell upwind method (Godunov)
     ! print*,"bar ht", bar_ht
    ! bar_height = 0.5d0!bar_ht
    ! bar_index = 50!bar_index_i
    ! bar_l = 0.502d0!bar_loc
    ! write(*,*) "BARHT:", bar_ht

    dtdx = dtdx1d(bar_index_i)
    amdq_wall = 0.d0
    apdq_wall = 0.d0


     lamb= (2*alpha)/(1+alpha) ! instead of alpha's below, alternatively, you can replace with lamb
     qh_L =(1-alpha)*q1d(:,bar_index_i-1) + alpha*q1d(:,bar_index_i)!q1d(:,bar_index_i)! these are for barrier ON grid edge problem
     qh_R = (1-alpha)*q1d(:,bar_index_i+1) + alpha*q1d(:,bar_index_i+2)!q1d(:,bar_index_i+1)!
     auxh_L = (1-alpha)*aux2(1,bar_index_i-1) + alpha*aux2(1,bar_index_i)!aux2(1,bar_index_i)!
     auxh_R =(1-alpha)*aux2(1,bar_index_i+1) + alpha*aux2(1,bar_index_i+2)!aux2(1,bar_index_i+1)!
    if (ixy == 1 .and. vert) then
      ! write(*,*) "q1d : ",q1d(1:2,bar_index),q1d(1:2,bar_index+1)
      call redistribute_fwave(ixy,qh_L,qh_R,auxh_L,auxh_R,bar_ht,1,&
      wave_wall,s_wall,amdq_wall,apdq_wall,3,3)
      amdq(:,bar_index_i+1) = amdq_wall
      apdq(:,bar_index_i+1) = apdq_wall
      s(:,bar_index_i+1) = s_wall
    end if

    if (ixy == 2 .and. horz) then
      ! write(*,*) "q1d : ",q1d(1:2,bar_index_i),q1d(1:2,bar_index_i+1)
      call redistribute_fwave(ixy,qh_L,qh_R,auxh_L,auxh_R,&
      bar_ht,1,wave_wall,s_wall,amdq_wall,apdq_wall,3,3)
      amdq(:,bar_index_i+1) = amdq_wall
      apdq(:,bar_index_i+1) = apdq_wall
      s(:,bar_index_i+1) = s_wall
    end if
   ...
   qadd(m,i-1) = qadd(m,i-1) - dtdx1d(i-1)*amdq(m,i)
     ...
     dtdx1d(bar_index_i) = dtdx
     dtdx1d(bar_index_i+1) = dtdx
