...
class ClawSolver2D(ClawSolver):
    ...
    def step_hyperbolic(self,solution):
        ...
        if(self.kernel_language == 'Fortran'):
            ...
            bar_index_i = state.problem_data['bar_index_i']
            bar_ht = state.problem_data['bar_ht']
            bar_loc = state.problem_data['bar_loc']
            alpha = state.problem_data['alpha']

            # oriention of barrier problem data:
            if state.problem_data['orientation'] == 'vert':
                vert=True
                horz=False
            elif state.problem_data['orientation'] == 'horz':
                horz=True
                vert=False
            ...
            if self.dimensional_split:
                ...
                if state.problem_data['method'] == 'hbox':
                    self.qbc, cfl_x = self.fmod.step2ds(maxm,self.num_ghost,mx,my, \
                          ...
                          self.aux1,self.aux2,self.aux3,self.work,1,self.fwave,rpn2,rpt2,vert,horz, \
                    ...
                    self.qbc, cfl_y = self.fmod.step2ds(maxm,self.num_ghost,mx,my, \
                          ...
                          self.aux1,self.aux2,self.aux3,self.work,2,self.fwave,rpn2,rpt2,vert,horz, \
                          bar_index_i,bar_ht,bar_loc)
