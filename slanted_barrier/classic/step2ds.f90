...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    integer :: upS_cells_i(mx*4),upS_cells_j(mx*4)
    ...
    real(kind=8)::x_0,x_e,y_0,y_e,alpha,bar_height,bar_ht
    ...
    integer :: i1,j1,i2,j2
    real(8) :: a1,a2,cfl2,qval(3),qval2(3),q_hbox_d(3,mx*4),q_hbox_u(3,mx*4)
    real(8) :: q_inc_d(3,1-num_ghost:mx+num_ghost,1-num_ghost:my+num_ghost)
    real(8) :: q_inc_u(3,1-num_ghost:mx+num_ghost,1-num_ghost:my+num_ghost)
    real(8) :: fwave(3,3,mx*4), s(3,mx*4), amdq_t(3,mx*4),apdq_t(3,mx*4)
    real(8) :: hbox_areas_d(mx*4) ,hbox_areas_u(mx*4),indices_data_area_d(4,8*mx)
    real(8) :: indices_data_area_u(4,8*mx)
    ...
    real(8) :: area_frags_u(4,mx*4), area_frags_d(4,mx*4)
...
!     # flux2 routine.  Find starting index of each piece:
    ...
    open (unit=1,file="./small_cells_data.txt",FORM="FORMATTED",STATUS="OLD",&
    ACTION="READ",access='sequential')
    rewind 1
    read(1,fmt=*,iostat=iostatus,end=101) N
    read(1,FMT=*,end=101) ii(1:N)
    read(1,fmt=*,end=101) jj(1:N)
    read(1,fmt=*,end=101) intersections(:,1:N+1)
    read(1,fmt=*,end=101) type_supper(1:N)
    read(1,fmt=*,end=101) type_sunder(1:N)
    read(1,fmt=*) lengths_supper(:,1:N)
    read(1,fmt=*) lengths_sunder(:,1:N)
    read(1,fmt=*) area_supper(1:N)
    read(1,fmt=*) area_sunder(1:N)
    read(1,fmt=*) xlower
    read(1,fmt=*) ylower
    read(1,fmt=*) xupper
    read(1,fmt=*) yupper
    ...
    read(1,fmt=*) x_e
    read(1,fmt=*) y_e
    read(1,fmt=*) k_count_up
    read(1,fmt=*) k_count_un
    read(1,fmt=*) all_undercells_i(1:k_count_un)
    read(1,fmt=*) all_undercells_j(1:k_count_un)
    read(1,fmt=*) all_uppercells_i(1:k_count_up)
    read(1,fmt=*) all_uppercells_j(1:k_count_up)
    read(1,fmt=*) N_ij_up
    read(1,fmt=*) N_ij_un
    read(1,fmt=*) unS_cells_i(1:N)
    read(1,fmt=*) unS_cells_j(1:N)
    read(1,fmt=*) upS_cells_i(1:N)
    read(1,fmt=*) upS_cells_j(1:N)
    read(1,fmt=*) up_area_ij
    read(1,fmt=*,end=101) un_area_ij
    close (1) !,status="keep")
  ...
  read(2,*) whose_d
  read(2,*) whose_u
  read(2,*) uniq_indices_d
  read(2,*) uniq_indices_u
  read(2,*) indices_data_d
  read(2,*) indices_data_u
  read(2,*) indices_data_area_d
  read(2,*) indices_data_area_u
  read(2,*) counts_d
  read(2,*) counts_u
  read(2,*) i2u
  read(2,*) i2d
  read(2,*) comp_cov_inds_d
  read(2,*) comp_cov_inds_u
  read(2,*) C2d
  read(2,*,end=113) C2u
  close(2)!,status="keep")
  113 continue
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !     # sweep in the y-direction.
        ...
        !        # compute modifications fadd and gadd to fluxes along this slice:
            ...
            fm1(:,:,j) = amdq
            ! print*, "q1d2: ",q1d2
            call rpt22(1,1,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
            aux1,aux2l,aux3,amdq,bmasdq,bpasdq)
            ...
            call rpt22(1,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
            aux1,aux2l,aux3,apdq,bmasdq,bpasdq)
            ...
            cfl = dmax1(cfl,max(cfl1d,cfl1d2))
            ! cfl = dmax1(cfl,cfl1d2)
        !     # split the right-going flux difference into down-going and up-going:
            call rpt22(1,1,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d2,q1d2, &
            aux1,aux2u,aux3,amdq,bmasdq2,bpasdq2)

        !     # modify flux below and above by B^- A^+ Delta q and  B^+ A^+ Delta q:
        forall ( m=1:num_eqn , i = 1:mx+1)
        gadd2(m,1,i-1) = gadd2(m,1,i-1) - &
        0.5d0*dtdx1d(i-1) * bmasdq2(m,i)
        gadd2(m,2,i-1) = gadd2(m,2,i-1) - &
        0.5d0*dtdx1d(i-1) * bpasdq2(m,i)
        end forall
            call rpt22(1,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d2,q1d2, &
            aux1,aux2u,aux3,apdq,bmasdq2,bpasdq2)

        !     # modify flux below and above by B^- A^+ Delta q and  B^+ A^+ Delta q:
            forall ( m=1:num_eqn , i = 1: mx+1)
            gadd2(m,1,i) = gadd2(m,1,i) - &
            0.5d0*dtdx1d(i) * bmasdq2(m,i)
            gadd2(m,2,i) = gadd2(m,2,i) - &
            0.5d0*dtdx1d(i) * bpasdq2(m,i)
            end forall
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !     ==================
        ...
        !        # compute modifications fadd and gadd to fluxes along this slice:
            ...
            call rpt22(2,1,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
            aux1,aux2l,aux3,amdq,bmasdq,bpasdq)
            ...
            call rpt22(2,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
            aux1,aux2l,aux3,apdq,bmasdq,bpasdq)
            ...
            call rpt22(2,1,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d2,q1d2, &
            aux1,aux2u,aux3,amdq,bmasdq2,bpasdq2)
        ...
        !     # modify flux below and above by B^- A^+ Delta q and  B^+ A^+ Delta q:
            ...
            call rpt22(2,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d2,q1d2, &
            aux1,aux2u,aux3,apdq,bmasdq2,bpasdq2)
    ...
    ! Correct to get the Q^ for both sides of the barrier, for small cells
      ...
      call small_cells_update_correct(ii,jj,dtdx,qold,qold2,qnew,qnew2,aux(1,:,:),&
      intersections(:,1:N+1),type_supper(1:N),type_sunder(1:N),lengths_supper(:,1:N),&
      lengths_sunder(:,1:N),area_sunder(1:N),area_supper(1:N),ids,N,xlower,ylower,&
      xupper,yupper,mx,my,num_ghost,x_0,y_0,x_e,y_e,bar_height,fm1,fp1,gm1,gp1,fm2,&
      fp2,gm2,gp2,ot,q_hbox_d,q_hbox_u,hbox_areas_d,hbox_areas_u,comp_cov_inds_d(:,1:C2d),&
      comp_cov_inds_u(:,1:C2u))  ! edit this routine to get hbox values updated with a*dq_wall
      ! print*, "AFTER:", q_hbox_d(1,1:10)

      ! Incremental q:
      q_inc_d = q_inc_d + qnew
      q_inc_u = q_inc_u + qnew2

      ! get down and up hboxes & update them ! rotate qhbox appropriately
      call rn2(2,N,3,3,1,2,mx,q_hbox_d(:,1:N),q_hbox_d(:,1:N),aux1(1,1:N),aux1(1,1:N),&
          fwave(:,:,1:N),s(:,1:N),amdq_t(:,1:N),apdq_t(:,1:N))
      ! print *, "APDQ:",apdq_t(:,1:N)
      do i =2,N
        q_hbox_d(:,i) = q_hbox_d(:,i)- dx*dtdx/lengths_sunder(1,i) * apdq_t(:,i) !/hbox_areas_d(i)
        q_hbox_d(:,i-1) = q_hbox_d(:,i-1) - dx*dtdx/lengths_sunder(1,i)*amdq_t(:,i)!/hbox_areas_d(i-1)
      end do
      call rn2(2,N,3,3,1,2,mx,q_hbox_u(:,1:N),q_hbox_u(:,1:N),aux1(1,1:N),aux1(1,1:N),&
          fwave(:,:,1:N),s(:,1:N),amdq_t(:,1:N),apdq_t(:,1:N))
      do i =2,N
        q_hbox_u(:,i) = q_hbox_u(:,i)- dx*dtdx/lengths_supper(1,i)* apdq_t(:,i)!/hbox_areas_u(i)
        q_hbox_u(:,i-1) = q_hbox_u(:,i-1) - dx*dtdx/lengths_supper(1,i)*amdq_t(:,i)!/hbox_areas_u(i-1)
      end do
      call hbox_SRD_update(mx,q_hbox_d(:,1:N),hbox_areas_d(1:N),N_hbox_d(1:N),whose_d(1:N))
      call hbox_SRD_update(mx,q_hbox_u(:,1:N),hbox_areas_u(1:N),N_hbox_u(1:N),whose_u(1:N))
      call rotate_qhbox_orig(N,q_hbox_d(:,1:N),q_hbox_d(:,1:N),intersections(:,1:N+1),ot)
      call rotate_qhbox_orig(N,q_hbox_u(:,1:N),q_hbox_u(:,1:N),intersections(:,1:N+1),ot)
      call conserve_correct(q_hbox_d(:,1:N),num_frags_d(1:N),index_frags_d(:,:,1:N),area_frags_d(:,1:N),&
        comp_cov_inds_d(:,1:C2d),q_inc_d,hbox_areas_d(1:N),mx,my,num_ghost,dx,dy,un_area_ij)
      call conserve_correct(q_hbox_u(:,1:N),num_frags_u(1:N),index_frags_u(:,:,1:N),area_frags_u(:,1:N),&
        comp_cov_inds_u(:,1:C2u),q_inc_u,hbox_areas_u(1:N),mx,my,num_ghost,dx,dy,up_area_ij)



      call grid_update_hbox(mx,my,num_ghost,dx,dy,q_hbox_d(:,1:N),q_hbox_u(:,1:N),qnew,qnew2,&
         uniq_indices_d(:,1:i2d),uniq_indices_u(:,1:i2u),&
        indices_data_area_d(:,1:i2d),indices_data_area_u(:,1:i2u),un_area_ij,up_area_ij,&
        counts_d(1:i2d),counts_u(1:i2u),&
        comp_cov_inds_d(:,1:C2d),comp_cov_inds_u(:,1:C2u),hbox_areas_d(1:N),&
        hbox_areas_u(1:N),indices_data_u(:,1:i2u),&
        indices_data_d(:,1:i2d))
        ! print *, "GET HERE"
      ...
      do i=1,N
        ...
        a1 = un_area_ij(i1,j1)
        a2 = up_area_ij(i2,j2)
        if ((abs(a1-0.5d0).lt.5d-13 .or. a1.gt.0.5d0) .and. a1.ne.1.d0) then
          qnew(:,i1,-1:j1-1) = qnew2(:,i1,-1:j1-1)
        else
          qnew(:,i1,-1:j1) = qnew2(:,i1,-1:j1)
        end if
        if ((abs(a2-0.5d0).lt.5d-13 .or. a2.gt.0.5d0) .and. a2.ne.1.d0) then
          qnew2(:,i2,j2+1:my+2) = qnew(:,i2,j2+1:my+2)
        else
          qnew2(:,i2,j2:my+2) = qnew(:,i2,j2:my+2)
        end if
      end do
    end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine rn2(ixy,maxm,meqn,mwaves,maux,mbc,mx,ql,qr,auxl,auxr,wave,s,amdq,apdq)
! =====================================================
...
!       3 y-momentum
    ...
    double precision, dimension(meqn,1:maxm), intent(in) :: ql, qr
    double precision, dimension(maux,1:maxm), intent(in) :: auxl, auxr
    double precision, dimension(meqn, mwaves, 1:maxm), intent(out) :: wave
    double precision, dimension(mwaves, 1:maxm), intent(out) :: s
    double precision, dimension(meqn, 1:maxm), intent(out) :: amdq, apdq
...
!              endif
!           enddo
!           print *, "i", i, "s: ", s(:,i), "fwave: ", fwave(:,:,i)
!           print *, "i", i , "APDQ: ", apdq(:,i)
!        enddo
!
!     return
!     end subroutine
