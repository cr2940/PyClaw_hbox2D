...
class Solver(object):
    ...
    def __init__(self,riemann_solver=None,claw_package=None):
        ...
        self.qbc = None   # the lower grid
        self.qbc2 = None  # the upper grid
        self.auxbc = None # the lower grid
        self.auxbc2 = None # the upper grid
        self.rp = None
        self.fmod = None
    ...
    def _allocate_bc_arrays(self,state):
        ...
        qbc2_dim = [n+2*self.num_ghost for n in state.grid.num_cells]
        qbc2_dim.insert(0,state.num_eqn)
        self.qbc2 = np.zeros(qbc2_dim,order='F')
        ...
        auxbc2_dim = [n+2*self.num_ghost for n in state.grid.num_cells]
        auxbc2_dim.insert(0,state.num_aux)
        self.auxbc2 = np.empty(auxbc2_dim,order='F')
    ...
    def _apply_bcs(self, state):
        ...
        self.qbc,self.qbc2 = state.get_qbc_from_q(self.num_ghost, self.qbc,self.qbc2)
        if state.num_aux > 0:
            self.auxbc,self.auxbc2 = state.get_auxbc_from_aux(self.num_ghost, self.auxbc,self.auxbc2)
