...
class ClawSolver2D(ClawSolver):
    ...
    def step_hyperbolic(self,solution):
            ...
            if self.dimensional_split:
                ...
                if state.problem_data['method'] == 'hbox':
                    self.qbc,self.qbc2,cfl_x = self.fmod.step2ds(maxm,self.num_ghost,mx,my, \
                          qold,self.qbc,qold2,self.qbc2,self.auxbc,self.auxbc2,dx,dy,self.dt,self._method,self._mthlim,\
                          self.aux1,self.aux2,self.aux3,self.work,1,self.fwave,rpn2,rpt2,bar_ht)

                    self.qbc2 = self.boundary_qbc2(state.problem_data['BC2'],mx,my,1)


                    self.qbc,self.qbc2,cfl_y = self.fmod.step2ds(maxm,self.num_ghost,mx,my, \
                          self.qbc,self.qbc,self.qbc2,self.qbc2,self.auxbc,self.auxbc2,dx,dy,self.dt,self._method,self._mthlim,\
                          self.aux1,self.aux2,self.aux3,self.work,2,self.fwave,rpn2,rpt2,bar_ht)
                    self.qbc2 = self.boundary_qbc2(state.problem_data['BC2'],mx,my,2)
        ...
        return self.qbc2
    def boundary_qbc2(self,BC,mx,my,ixy):
        if BC=="ww" and ixy == 1:
            self.qbc2[0,1,:] = self.qbc2[0,2,:]
            self.qbc2[0,0,:] = self.qbc2[0,3,:]
            self.qbc2[1:3,1,:] = -self.qbc2[1:3,2,:]
            self.qbc2[1:3,0,:] = -self.qbc2[1:3,3,:]

        if BC=="ww" and ixy == 2:
            self.qbc2[1:3,:,mx+2] = -self.qbc2[1:3,:,mx+1]
            self.qbc2[1:3,:,mx+3] = -self.qbc2[1:3,:,mx]
            self.qbc2[0,:,mx+2] = self.qbc2[0,:,mx+1]
            self.qbc2[0,:,mx+3] = self.qbc2[0,:,mx]

        if BC=="ee" and ixy == 1:
            self.qbc2[0,1,:] = self.qbc2[0,2,:]
            self.qbc2[0,0,:] = self.qbc2[0,1,:]
            self.qbc2[1:3,1,:] = self.qbc2[1:3,2,:]
            self.qbc2[1:3,0,:] = self.qbc2[1:3,1,:]
            
        if BC=="ee" and ixy == 2:
            self.qbc2[1:3,:,mx+2] = self.qbc2[1:3,:,mx+1]
            self.qbc2[1:3,:,mx+3] = self.qbc2[1:3,:,mx+2]
            self.qbc2[0,:,mx+2] = self.qbc2[0,:,mx+1]
            self.qbc2[0,:,mx+3] = self.qbc2[0,:,mx+2]
